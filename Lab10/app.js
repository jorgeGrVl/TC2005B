            ...
            <div class="columns">
                <div class="column">
                    <form action="/" method="POST">
                        <label for="nombre" class="label">Escribe tu nombre:</label>
                        <input class="input is-primary" type="text" placeholder="Jorge" id="nombre" name="nombre"/>
                        <br><br>
                        <input type="submit" id="boton_personaje" class="button is-primary" value="Guardar">
                    </form>
                </div>
            </div>
`;
...
const server = http.createServer((request, response) => {
    if (request.url == "/" && request.method == "GET") {
        ...
        response.write(html_header + "<h2>Ruta 1</h2><br>" + html_content_form);
        response.end();
    }
    else if (request.url == "/ruta2" && request.method == "GET") {
        console.log(request.url);
        response.setHeader('Content-Type', 'text/html');
        response.write(html_header + "<h2>Ruta 2</h2>");
        response.end();
    }
    else if (request.url == "/ruta3" && request.method == "GET") {
        console.log(request.url);
        response.setHeader('Content-Type', 'text/html');
        response.write(html_header + "<h2>Ruta 3</h2>");
        response.end();
    }
        ...
        response.setHeader('Content-Type', 'text/html');
        response.write(html_header + "<h2>La página no existe</h2>");
        response.end();
    }
})
